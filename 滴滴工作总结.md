## 技术栈
前端：基本功（html,css,javascript,etc），React全家桶，Vue全家桶，webpack  
服务端：java spring boot，nodejs，SQL  

## RLAB工作（2019-02~2020-07）
岗位：高级前端工程师

### Koa Common项目（2019-02~2019-05）

Koa Common是基于koa的公共组件库，为了统一外卖业务线的nodejs技术栈，提高业务开发维护效率和人员互备能力。我负责以下模块的接口设计和开发工作，并为每个模块写了完善的单元测试，
有规范的代码注释可以用jsdoc自动生成接口文档。最终此项目在两个业务项目上成功落地。  

1. SSO模块：内部员工SSO登录
2. Request模块：发送携带trace信息的http请求
3. ORM模块：数据库连接、Model定义、SQL构建
4. Validation模块：接口参数校验
5. 日志模块：打符合didi log规范的日志
6. disf模块：用来接入disf
7. thrift模块：发送符合rlab规范的thrift请求
8. etc

### 时光机项目（2019-05~2020-07）

时光机是外卖项目的用户微观行为分析工具，辅助PM和运营做决策，辅助RD分析问题，下图体现了时光机的项目定位。
![Screen Shot 2021-03-04 at 8 19 32 PM](https://user-images.githubusercontent.com/21496977/109963175-0ff76480-7d27-11eb-9782-7c1e88de8199.png)


时光机项目包含标签筛选，用户活跃度图表，场景还原，用户画像等核心功能，下图展示了相关技术模块，我负责整个大前端部分的开发，项目的技术难点主要是场景还原部分，时光机项目在2020年主要服务于外卖项目，共辅助解决17个产品体验问题，为分析疑难问题和优化产品体验提供了有力的抓手。
![Screen Shot 2021-03-04 at 8 19 04 PM](https://user-images.githubusercontent.com/21496977/109963169-0e2da100-7d27-11eb-91aa-bee6fcd7b5b0.png)

## IBT safety platform（2019-02~2020-07）
岗位：高级全栈工程师

### Clipper知识库
Clipper中建设知识库的储存和搜索功能（第一期），并关联知识库与特定工单（第二期），提高一线处理工单时知识库定位效率，帮助中台进行知识库管理和有效透传

1. 方案调研：
2. 方案设计：对比我们的需求和开源版wiki现状，合理拆解二次开发的工作量，与Clipper权限打通、国家空间、改造成wiki用户习惯的目录树设计
3. 用户体验：用户反馈良好，目录导航改造成符合滴滴wiki用户使用习惯的目录树，基于CKEditor定制的功能丰富的编辑器，无论视觉和功能都能达到业界一流水平
4. 工作质量：在无QA情况下，主动自测，上线后用户反馈的bug屈指可数，并在大面积铺开使用的情况下依然稳定。
5. 业务收益：已开所有国家，墨西哥和中北美两个地区已全面迁移到Clipper知识库，其它国家也正在搭建，目前文章数150+，保障了信息安全，节约了费用支出，对客服提效有所提升，并为未来通过工单关联知识库大幅客服提效奠定根基。同时，知识库也有望对RD团队的oncall痛点有所帮助，目前正在搭建FAQ。
6. 知识分享：整理知识库runbook，让团队成员都能对知识库项目有全面的了解，让项目能可持续发展（http://wiki.intra.xiaojukeji.com/pages/editpage.action?pageId=466494902）


1. 方案选型
充分调研了公司内部、IBT部门内部、以及业界的知识库方案，包括接入现有产品的可行性分析，自主开发的成本分析，最终确定了基于开源wiki做二次开发的方案，使投入产出比最大化。
<img width="985" alt="Screen Shot 2021-03-06 at 8 32 15 PM" src="https://user-images.githubusercontent.com/21496977/110206849-23d3cf80-7ebb-11eb-88cd-110a36134378.png">

2. 需求拆解
根据产品需求和开源wiki现状分析，确定有哪些需要做的事，难度和工作量有多大。下图绿色部分是开源wiki现成的，黄色部分是1期需要开发的功能，红色部分是2期需要开发的功能。
<img width="610" alt="Screen Shot 2021-03-06 at 8 38 04 PM" src="https://user-images.githubusercontent.com/21496977/110206978-fe939100-7ebb-11eb-8811-8cfe71e4b888.png">

3. 开发实现
排期1个月，先开发一期功能，开发过程比较顺利，结果符合预期。下图是主要技术栈：
<img width="622" alt="Screen Shot 2021-03-06 at 8 38 35 PM" src="https://user-images.githubusercontent.com/21496977/110206980-02271800-7ebc-11eb-97dd-965d4065ba61.png">

4. 项目成果
Clipper知识库在已开国的所有国家落地（除巴西），获得广泛应用和积极评价。

美东服务：
<img width="759" alt="Screen Shot 2021-03-06 at 8 47 30 PM" src="https://user-images.githubusercontent.com/21496977/110207315-9cd42680-7ebd-11eb-8139-a83e7cd3ef02.png">

俄罗斯服务：
<img width="761" alt="Screen Shot 2021-03-06 at 8 48 01 PM" src="https://user-images.githubusercontent.com/21496977/110207316-9e9dea00-7ebd-11eb-8e7f-3b34b270ea68.png">

最近三日PV/UV（3.2-3.4）：
<img width="1153" alt="Screen Shot 2021-03-06 at 8 53 15 PM" src="https://user-images.githubusercontent.com/21496977/110207400-3ef40e80-7ebe-11eb-88fd-56a08edde7ac.png">

项目质量：
上线4个月以来，用户反馈的问题屈指可数，维护成本极小，下图是前端错误统计，几乎为0：
<img width="1196" alt="Screen Shot 2021-03-06 at 8 54 00 PM" src="https://user-images.githubusercontent.com/21496977/110207410-45828600-7ebe-11eb-96e6-0b96bfd95eb9.png">

### 审批中心



